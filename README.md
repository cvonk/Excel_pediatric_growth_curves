...
Input is in metric. Imperial units are shown on the right.
...
Make sure there are no empty dates in the table, otherwise the graphs will start at 1.
...
The procentile curves are based on CDC data for boys.  Feel free to switch to girls of course.
To switch to girls, click on a curve and "Select Data", then for each P curve, e.g.:
